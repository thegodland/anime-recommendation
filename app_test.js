  ...
  connection.then( async (conn)=>{
    ...
    var predictRatingList = unratedID.reduce( async (previousPromise,unAid,index,array)=>{
      ...
      var similarityWithRated = ratedID.map( async (aid)=>{
        ...
        var mutualRawData = await Promise.all([connectMysql.query(ratedmutualQuery), connectMysql.query(unratedmutualQuery), connectMysql.query(mutualUserQuery)]);
        ...
        // console.log(mutualRawData[2]);
          ...
          if (mutualRawData[0].length<=3 || mutualRawData[1].length<=3){
            ...
            let ratedMutualRating = mutualRawData[0].map( (val,index)=>{ return (val.rating-avgPerUserRating[index]); });
            let unratedMutualRating = mutualRawData[1].map( (val,index)=>{ return (val.rating-avgPerUserRating[index]); });
